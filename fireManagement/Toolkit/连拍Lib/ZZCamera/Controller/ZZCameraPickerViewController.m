...
@property (nonatomic, strong) NSMutableArray<UIImage *>    *cameraArray;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([[self.cameraArray objectAtIndex:indexPath.row] isKindOfClass:[UIImage class]]) {
        UIImage *camera = [self.cameraArray objectAtIndex:indexPath.row];
        [photoCell loadPhotoDatas:camera];
    }
...
- (void)Captureimage
{
    ...
    [self.captureOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:
     ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
         ...
         [weakSelf.cameraArray addObject:t_image];
         [weakSelf.picsCollection reloadData];
         [weakSelf.picsCollection scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:weakSelf.cameraArray.count - 1 inSection:0] atScrollPosition:UICollectionViewScrollPositionRight animated:YES];
     }];
}
...
{
    hudShowError([NSString stringWithFormat:Alert_Max_TakePhoto,(long)photoNumOfMax]);
}
