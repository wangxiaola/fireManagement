...
//								constrainedToSize:maxSize lineBreakMode:detailsLabel.lineBreakMode];
    ...
    NSDictionary *attribute = @{NSFontAttributeName: detailsLabel.font};

    
    CGSize expectedSizeMessage  = [detailsLabel.text boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:attribute context:nil].size;
    
    
	totalSize.width = MAX(totalSize.width, expectedSizeMessage.width);
	totalSize.height += expectedSizeMessage.height;
	if (expectedSizeMessage.height > 0.f && (indicatorF.size.height > 0.f || labelSize.height > 0.f)) {
		totalSize.height += kPadding;
	}
	...
	detailsLabelF.origin.x = roundf((bounds.size.width - expectedSizeMessage.width) / 2) + xPos;
	detailsLabelF.size = expectedSizeMessage;
